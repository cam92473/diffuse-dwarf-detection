...
def get_master_catalogs(data, weight_params, phot_filter, mag_range, reff_range, n_range, axisratio_range, theta_range, num_dwarfs, num_runs, psf, reff_units, positions, subtract, obj_params, masktype, maxdilations, maskfunc, windowsize, dolog, det_params, sigclip, match_tol, gallery, no_clean, verbose, outdir, sexdir, frpath, signature):
    ...
    if verbose:
        print(f"getting master catalogs for {num_runs} runs of mag {mag_range[0]}-{mag_range[1]} & reff {reff_range[0]}-{reff_range[1]}")
        t1 = time.perf_counter()
    ...
    for run_no in range(num_runs):
        ...
        get_detection_catalog(rundir/f'{signature}_filled.fits', weight_params, obj_params, masktype, maxdilations, maskfunc, windowsize, dolog, det_params, sigclip, gallery, verbose, rundir, sexdir, signature)    
        compare_catalogs(rundir,match_tol,signature,verbose)
        if not no_clean:
            clean_up_files(rundir, signature, verbose)
    create_master_catalogs(rundirs,num_runs,signature,verbose)
    if verbose:
        t2 = time.perf_counter()
        print(f"finished getting master catalogs. Total time: {t2-t1}")
...
if __name__ == "__main__":
    ...
    from catalog.compare_catalogs import compare_catalogs
