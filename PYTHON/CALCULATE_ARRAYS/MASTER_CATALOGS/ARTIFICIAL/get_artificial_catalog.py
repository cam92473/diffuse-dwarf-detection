...
def convert_reff_to_pix(reff_range,reff_units):
...
'''def convert_brightness_to_magnitudes(brightness_range,brightness_quantity):
    if brightness_quantity == 'mag':
        pass
    elif brightness_quantity == 'Ieff':
        pass
    elif brightness_quantity == 'SBeff':
        SBeffs = 5*log10(res)-2.5*log10(Ieffs)+zp'''
...
def get_artificial_catalog(data, phot_filter, mag_range, reff_range, n_range, axisratio_range, theta_range, num_dwarfs, psf, windowsize, reff_units, positions, subtract, gallery, verbose, outdir, frpath, signature):
    ...
    reff_range = convert_reff_to_pix(reff_range,reff_units)
    #convert_brightness_to_magnitudes(brightness_range,brightness_quantity)
    ...
    SBeffs = 5*log10(res)-2.5*log10(Ieffs)+zp
    SB0s = 5*log10(res)-2.5*log10(I0s)+zp
    ...
    artificial_catalog = np.array((x0s,y0s,mags,SBeffs,SB0s,reffs*0.2637,ns,axisratios,thetas,xs,ys)).T
    cols=['x0','y0','mag','SBeff','SB0','reff[pix]','n','axisratio','theta','x','y']
    art = pd.DataFrame(data=artificial_catalog,columns=cols)
    art.to_csv(outdir/f'{signature}_input_artificial_dwarfs.csv',index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('mag_range', nargs=2, type=float, help='Two numbers (low and high) that specify the range of the brightness of the artificial dwarfs.')
    #parser.add_argument('brightness_quantity', choices=['mag','Ieff','SBeff'], help='The brightness quantity specified in the previous argument. Can be apparent magnitudes ("mag"), effective intensity ("Ieff"), or the surface brightness at the effective radius ("SBeff"). Quantities will be converted to apparent magnitudes in order to uniformly distribute the dwarf brightnesses in terms of apparent magnitude.')
    ...
    parser.add_argument('-reff_units', choices=['as','px','pc'], default='as', help='The units of the previously inputted values for reff_range. Can be arcseconds ("as"), pixels ("px"), or parsecs ("pc"). The program converts between these values using the distance to Centaurus A (3.8E6 pc) and the resolution of DECam (0.2637 "/pix).')
    ...
    #brightness_quantity = args.brightness_quantity
