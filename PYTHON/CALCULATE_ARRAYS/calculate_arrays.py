...
from MASTER_CATALOGS.get_master_catalogs import get_master_catalogs
from plot_arrays import make_plot

def create_arrays(pixeldirs,num_rows,num_cols,signature):
    completeness_array = np.zeros((num_rows,num_cols))
    dwarfdetections_array = np.zeros((num_rows,num_cols))

    for r in range(num_rows):
        for c in range(num_cols):
            p = r*num_cols+c
            with open(pixeldirs[p]/f'{signature}_master_input_artificial_dwarfs.csv') as cat:
                lines = cat.readlines()
                num_input_dwarfs = len(lines)-1
            with open(pixeldirs[p]/f'{signature}_master_detected_artificial_dwarfs.csv') as cat:
                lines = cat.readlines()
                num_detected_dwarfs = len(lines)-1
            with open(pixeldirs[p]/f'{signature}_master_non_artificial_dwarf_detections.csv') as cat:
                lines = cat.readlines()
                num_nondwarfs = len(lines)-1
            completeness_array[r,c] = num_detected_dwarfs/num_input_dwarfs*100
            dwarfdetections_array[r,c] = num_detected_dwarfs/num_nondwarfs*100

    return completeness_array, dwarfdetections_array

def construct_arrays(data, weight_params, phot_filter, mag_bin_vals, reff_bin_vals, n_range, axisratio_range, theta_range, num_dwarfs, num_runs, psf, reff_units, positions, subtract, obj_params, masktype, maxdilations, maskfunc, windowsize, dolog, det_params, sigclip, match_tol, gallery, no_plot, no_clean, verbose, outdir, sexdir, frpath, saveimdir, signature):

    if verbose:
        print("Starting to build completeness and dwarfdetections array")
    ...
    for r in range(num_rows):
        for c in range(num_cols):
            ...
            get_master_catalogs(data, weight_params, phot_filter, mag_range, reff_range, n_range, axisratio_range, theta_range, num_dwarfs, num_runs, psf, reff_units, positions, subtract, obj_params, masktype, maxdilations, maskfunc, windowsize, dolog, det_params, sigclip, match_tol, gallery, no_clean, verbose, pixeldir, sexdir, frpath, signature)

    completeness_array, dwarfdetections_array = create_arrays(pixeldirs,num_rows,num_cols,signature)
    np.savetxt(outdir/f"{signature}_completeness.arr",completeness_array,header=f"{mag_bin_vals[0]} {mag_bin_vals[1]} {mag_bin_vals[2]} {reff_bin_vals[0]} {reff_bin_vals[1]} {reff_bin_vals[2]} {reff_units}")
    np.savetxt(outdir/f"{signature}_dwarfdetections.arr",dwarfdetections_array,header=f"{mag_bin_vals[0]} {mag_bin_vals[1]} {mag_bin_vals[2]} {reff_bin_vals[0]} {reff_bin_vals[1]} {reff_bin_vals[2]} {reff_units}")

    if verbose:
        t2 = time.perf_counter()
        print(f"finished building completeness and dwarfdetections arrays and saved them to output directory. Total time: {t2-t1}")

    savename = saveimdir/f"{signature}_results.png"
    if not no_plot:
        make_plot([mag_bins],[reff_bins],[reff_units],[num_rows],[num_cols],[completeness_array],[dwarfdetections_array],[signature],savename)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-masktype', default='nan', choices=['nan','bg'], help='Whether to mask out bright sources with the sextractor-calculated image background or NaNs.')
    ...
    parser.add_argument('--no_plot', action='store_true', default=False, help='Disables plotting the completion at the end of the program. Useful if you do not want the thread to be halted by matplotlib.')
    ...
    try:
        ...
        outdir.mkdir(parents=True)
    sexdir = Path(root/'PYTHON'/'CALCULATE_ARRAYS'/'MASTER_CATALOGS'/'DETECTION'/'sextractor')
    frpath = Path(root/'PYTHON'/'CALCULATE_ARRAYS'/'MASTER_CATALOGS'/'ARTIFICIAL'/'DEC_filter_response.txt') 
    saveimdir = Path(root/'RESULTS'/'results_plots')

    with open(outdir/f'{signature}_cmdline_args.txt', 'w') as f:
        json.dump(args.__dict__, f, indent=2)

    construct_arrays(data, weight_params, phot_filter, mag_bin_vals, reff_bin_vals, n_range, axisratio_range, theta_range, num_dwarfs, num_runs, psf, reff_units, positions, subtract, obj_params, masktype, maxdilations, maskfunc, windowsize, dolog, det_params, sigclip, match_tol, gallery, no_plot, no_clean, verbose, outdir, sexdir, frpath, saveimdir, signature)
